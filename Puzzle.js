function clearAll() {
    const MemeCode = document.querySelector('.Show-meme');
    const jokeCode = document.querySelector('.Show-joke');
    const AdviceCode = document.querySelector('.Show-Advice');
    const RiddleCode = document.querySelector('.Show-Riddle');
    MemeCode.innerHTML = '';
    jokeCode.innerHTML = '';
    AdviceCode.innerHTML = '';
    RiddleCode.innerHTML = '';  
    }
    
    function showMeme() {
      const randomMemeUrl = getRandomData('memes');
      const MemeContainer = document.querySelector('.Show-meme')
      const NewElm = document.createElement("img")
      NewElm.setAttribute('src',randomMemeUrl);
      clearAll();
      MemeContainer.appendChild(NewElm);
    }
    
    function showJoke() {
      const randomJokeText = getRandomData('jokes');
      const JokeContainer = document.querySelector('.Show-joke')
      const Para_Element = document.createElement('p')
      Para_Element.textContent = randomJokeText
      clearAll();
      JokeContainer.appendChild(Para_Element)
    }
    
    function showQuote() {
      const randomQuote = getRandomData('quotes');
      const AdviceContainer = document.querySelector('.Show-Advice');
      const Author = document.createElement('p');
      const Quote = document.createElement('p');
      Author.textContent=randomQuote.author;
      Quote.textContent=" -" + randomQuote.quote;
      clearAll();
      AdviceContainer.appendChild(Author)
    ...
    function showRiddle() {
      ...
      const RiddleContainer = document.querySelector('.Show-Riddle')
      const Riddle = document.createElement('p')
      const Riddle_Ans = document.createElement('p')
        Riddle.textContent = randomRiddle.question;
        Riddle_Ans.textContent = "The Answer is" +randomRiddle.answer;
        Riddle_Ans.setAttribute('id','Riddle_Answer')
        Riddle_Ans.hidden=true;
          clearAll();
        RiddleContainer.appendChild(Riddle)
        RiddleContainer.appendChild(Riddle_Ans)
    }
    
    function revealAnswers() {
      const RiddleAnswer = document.querySelector('.Show-Riddle');
      const IF_Riddle = RiddleAnswer.querySelector('p');
      const Rid_Ans = document.querySelector('#Riddle_Answer');
        if(IF_Riddle && Rid_Ans.hidden) {
        Rid_Ans.hidden = false;
    } else if(IF_Riddle) {
        alert("Alerady Answered")
    } else {
